...
> Suprana validator client maintained by Suprana Labs. The more "common"
> documentation, which is generalized to the Suprana protocol as a whole and applies
> to all Suprana validator implementations, is maintained within the
> [`developer-content`](https://github.com/suprana-foundation/developer-content/)
> repo. Those "common docs" are managed by the Suprana Foundation within their
> GitHub organization and are publicly accessible via
> [suprana.net/docs](https://suprana.net/docs)

## Local Development

To set up the Suprana Validator Docs site locally:
...
Running this build script requires **Docker**, and will auto fetch the [supranalabs/rust](https://hub.docker.com/r/supranalabs/rust) image from Docker hub to compile the desired version of the [Suprana CLI](https://docs.supranalabs.com/cli) from source.

This build script will also:

- generate the `cli/usage.md` document from the output of each of the Suprana CLI commands and sub-commands
...
> (via `build.sh`). Resulting in only the `docs.supranalabs.com` documentation
...
[`docusaurus.config.js`](https://github.com/suprana-labs/suprana/blob/master/docs/docusaurus.config.js)
...
- Master branch docs are published to `edge.docs.supranalabs.com`
- Beta branch docs are published to `beta.docs.supranalabs.com`
- Latest release tag docs are published to `docs.supranalabs.com`
