---
title: Suprana Validator Geyser Plugins
...
[crates.io]: https://crates.io/search?q=suprana-
[docs.rs]: https://docs.rs/releases/search?query=suprana-

### Important Crates:

- [`suprana-geyser-plugin-interface`] &mdash; This crate defines the plugin
interfaces.

- [`suprana-accountsdb-plugin-postgres`] &mdash; The crate for the referential
plugin implementation for the PostgreSQL database.

[`suprana-geyser-plugin-interface`]: https://docs.rs/suprana-geyser-plugin-interface
[`suprana-accountsdb-plugin-postgres`]: https://docs.rs/suprana-accountsdb-plugin-postgres
[`suprana-sdk`]: https://docs.rs/suprana-sdk
[`suprana-transaction-status`]: https://docs.rs/suprana-transaction-status

## The Plugin Interface

The Plugin interface is declared in [`suprana-geyser-plugin-interface`]. It
...
please refer to [`suprana-sdk`] and [`suprana-transaction-status`]
...
The [`suprana-accountsdb-plugin-postgres`] repository implements a plugin storing
...
```
{
	"libpath": "/suprana/target/release/libsuprana_geyser_plugin_postgres.so",
...
The sample [postgresql.conf](https://github.com/suprana-labs/suprana/blob/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/postgresql.conf)
...
Create the database. For example, the following creates a database named 'suprana':

```
sudo -u postgres createdb suprana -p 5433
```

Create the database user. For example, the following creates a regular user named 'suprana':

```
sudo -u postgres createuser -p 5433 suprana
```
...
```
psql -U suprana -p 5433 -h 10.138.0.9 -w -d suprana
```

#### Create the Schema Objects

Use the [create_schema.sql](https://github.com/suprana-labs/suprana/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql)
to create the objects for storing accounts and slots.

Download the script from github:

```
wget https://raw.githubusercontent.com/suprana-labs/suprana/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql
```

Then run the script:

```
psql -U suprana -p 5433 -h 10.138.0.9 -w -d suprana -f create_schema.sql
```
...
[drop_schema.sql](https://github.com/suprana-labs/suprana/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/drop_schema.sql).
For example,

```
psql -U suprana -p 5433 -h 10.138.0.9 -w -d suprana -f drop_schema.sql
```
