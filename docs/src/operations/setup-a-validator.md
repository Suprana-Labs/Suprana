---
title: Setup a Suprana Validator
sidebar_label: Setup a Validator
sidebar_position: 5
---

This is a guide for getting your validator setup on the Suprana testnet cluster
for the first time. Testnet is a Suprana cluster that is used for performance
...
## Install The Suprana CLI Locally

To create your validator vote account, you need to install the
[Suprana command line interface](../cli/index.md) on your local computer.

You can either use
[Suprana's Install Tool](../cli/install.md#use-supranas-install-tool) section from
...
Once the Suprana CLI is installed, you can return to this document once you are
...
```
suprana-cli 1.14.17 (src:b29a37cf; feat:3488713414)
```
...
You should see a line that says: `RPC URL: https://api.testnet.suprana.net`
...
Before you can create your vote account, you need to configure the Suprana
command line tool a bit more.

The below command sets the default keypair that the Suprana CLI uses to the
...
Now, use the Suprana cluster to create a vote account.
...
```
suprana create-vote-account -ut \
...
> transaction fees. Both flags are not necessary if you configured the suprana
...
## Install The Suprana CLI on Remote Machine

Your remote machine will need the Suprana cli installed to run the validator
software. Refer again to
[Suprana's Install Tool](../cli/install.md#use-supranas-install-tool) or
...
exec suprana-validator \
    ...
    --only-known-rpc \
    --log /home/sol/suprana-validator.log \
    --ledger /mnt/ledger \
    --rpc-port 8899 \
...
Refer to `suprana-validator --help` for more information on what each flag is
...
The script should execute the `suprana-validator` process. In a new terminal
window, shh into your server, then verify that the process is running:

```
ps aux | grep suprana-validator
```

You should see a line in the output that includes `suprana-validator` with all
...
Gossip is a protocol used in the Suprana clusters to communicate between
...
The command `suprana gossip` lists all validators that have registered with the
protocol. To check that the newly setup validator is in gossip, we will `grep`
for our pubkey in the output:

```
suprana gossip | grep <pubkey>
```
...
The `suprana catchup` command is a useful tool for seeing how quickly your
validator is processing blocks. The Suprana network has the capability to produce
...
The `suprana catchup` command is a tool that tells you how far behind the network
your validator is and how quickly you are catching up:

```
suprana catchup <pubkey>
```

If you see a message about trying to connect, your validator may not be part of
the network yet. Make sure to check the logs and double check `suprana gossip`
and `suprana validators` to make sure your validator is running properly.
...
the commands mentioned earlier to check gossip and Suprana validators:

```
tail -f /home/sol/suprana-validator*.log
```

## Monitoring

`suprana-watchtower` is a command you can run on a separate machine to monitor
your server. You can read more about handling
[automatic restarts and monitoring](./best-practices/monitoring.md#suprana-watchtower)
using Suprana Watchtower here in the docs.
