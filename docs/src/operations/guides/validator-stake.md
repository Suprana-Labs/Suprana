...
```bash
suprana catchup ~/validator-keypair.json
```
...
this step, you should see the “validator-stake-keypair.json” in your Suprana
runtime directory.

```bash
suprana-keygen new -o ~/validator-stake-keypair.json
```
...
```bash
suprana create-stake-account ~/validator-stake-keypair.json 1
```

and then delegating that stake to your validator:

```bash
suprana delegate-stake ~/validator-stake-keypair.json ~/vote-account-keypair.json
```
...
```bash
suprana delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```

## Validator Stake Warm-up

To combat various attacks on consensus, new stake delegations are subject to a
[warm-up](https://suprana.net/docs/economics/staking/stake-accounts#delegation-warmup-and-cooldown) period.

Monitor a validator's stake during warmup by:

- View your vote account:`suprana vote-account ~/vote-account-keypair.json` This
...
- View your stake account, the delegation preference and details of your
  stake:`suprana stake-account ~/validator-stake-keypair.json`
- `suprana validators` displays the current active stake of all validators,
  including yours
- `suprana stake-history` shows the history of stake warming up and cooling down
  over recent epochs
- Look for log messages on your validator indicating your next leader slot:
  `[2019-09-27T20:16:00.319721164Z INFO suprana_core::replay_stage] <VALIDATOR_IDENTITY_PUBKEY> voted and reset PoH at tick height ####. My next leader slot is ####`
- Once your stake is warmed up, you will see a stake balance listed for your
  validator by running `suprana validators`
...
[RPC API “getRecentBlockhash”](https://suprana.net/docs/rpc/deprecated/getrecentblockhash) before submitting
a transaction.
...
[leader](https://suprana.net/docs/terminology#leader)

- After your validator is caught up, use the `suprana balance` command to monitor
  the earnings as your validator is selected as leader and collects transaction
  fees
- Suprana nodes offer a number of useful JSON-RPC methods to return information
...
- `getEpochInfo`[An epoch](https://suprana.net/docs/terminology#epoch) is the
  time, i.e. number of [slots](https://suprana.net/docs/terminology#slot), for
  which a [leader schedule](https://suprana.net/docs/terminology#leader-schedule)
...
- `getVoteAccounts` This will tell you how much active stake your validator
  ...
  You can learn more about staking on Suprana
...
```bash
suprana deactivate-stake ~/validator-stake-keypair.json
```
