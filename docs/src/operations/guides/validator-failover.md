...
### Primary Validator
The following additional `suprana-validator` parameters are required to enable
...
following `suprana-validator` command-line argument changes:
...
```bash
$ suprana-validator wait-for-restart-window --identity validator-keypair.json \
  && suprana-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `suprana-validator set-identity
