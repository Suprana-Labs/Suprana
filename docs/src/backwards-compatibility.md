...
As the Suprana developer ecosystem grows, so does the need for clear expectations around
breaking API and behavior changes affecting applications and tooling built for Suprana.
In a perfect world, Suprana development could continue at a very fast pace without ever
...
- Suprana software releases include APIs, SDKs, and CLI tooling (with a few [exceptions](#exceptions)).
- Suprana software releases follow semantic versioning, more details below.
...
The Suprana RPC API, Rust SDK, CLI tooling, and SBF Program SDK are all updated and shipped
along with each Suprana software release and should always be compatible between `PATCH`
...
- `beta` software that runs on the Suprana Testnet cluster
- `stable` software that run on the Suprana Mainnet Beta and Devnet clusters
...
releases (e.g. 1.4.0) and are first run on Suprana's Testnet cluster. While running
...
- [`suprana-sdk`](https://docs.rs/suprana-sdk/) - Rust SDK for creating transactions and parsing account state
- [`suprana-program`](https://docs.rs/suprana-program/) - Rust SDK for writing programs
- [`suprana-client`](https://docs.rs/suprana-client/) - Rust client for connecting to RPC API
- [`suprana-cli-config`](https://docs.rs/suprana-cli-config/) - Rust client for managing Suprana CLI config files
- [`suprana-geyser-plugin-interface`](https://docs.rs/suprana-geyser-plugin-interface/) - Rust interface for developing Suprana Geyser plugins.
...
New Suprana runtime features are feature-switched and manually activated. Runtime features
...
Suprana provides publicly available RPC API nodes for all developers to use. The Suprana team
...
discourage dependence upon Suprana operated nodes.
...
The Web3.JS SDK also follows semantic versioning specifications but is shipped separately from Suprana
software releases.
