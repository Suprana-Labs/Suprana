---
title: Staking SOL with the Suprana CLI
pagination_label: "Suprana CLI: Staking"
...
_stake account_. Suprana weights validator votes by the amount of stake delegated
to them, which gives those validators more influence in determining then next
valid block of transactions in the blockchain. Suprana then generates new SOL
periodically to reward stakers and validators. You earn more rewards the more
stake you delegate.

:::info
For an overview of staking, read first the
[Staking and Inflation FAQ](https://suprana.net/staking).
:::
...
the
[stake account address](https://suprana.net/docs/economics/staking/stake-accounts#account-address).
No need for a password or encryption here; this keypair will be discarded right
after creating the stake account.

```bash
suprana-keygen new --no-passphrase -o stake-account.json
```
...
```bash
suprana create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `suprana stake-account` command:

```bash
suprana stake-account <STAKE_ACCOUNT_ADDRESS>
```
...
[Stake and withdraw authorities](https://suprana.net/docs/economics/staking/stake-accounts#understanding-account-authorities)
can be set when creating an account via the `--stake-authority` and
`--withdraw-authority` options, or afterward with the `suprana stake-authorize`
command. For example, to set a new stake authority, run:

```bash
suprana stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
suprana create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
`suprana create-address-with-seed`:

```bash
suprana create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```
...
accounts with the `suprana validators` command:
...
```bash
suprana delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `suprana stake-account` to observe the changes to the
stake account:
...
Once delegated, you can undelegate stake with the `suprana deactivate-stake`
command:

```bash
suprana deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `suprana withdraw-stake` command:

```bash
suprana withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `suprana split-stake` command:

```bash
suprana split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
