---
title: Durable Transaction Nonces in the Suprana CLI
pagination_label: "Suprana CLI: Durable Transaction Nonces"
...
[`recent_blockhash`](https://suprana.net/docs/core/transactions#recent-blockhash).
They are implemented as a Suprana Program, the mechanics of which can be read
...
```bash
suprana-keygen new -o nonce-keypair.json
suprana create-nonce-account nonce-keypair.json 1
```
...
> [Full usage documentation](../usage.md#suprana-create-nonce-account)
...
```bash
suprana nonce nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#suprana-get-nonce)
...
```bash
suprana new-nonce nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#suprana-new-nonce)
...
```bash
suprana nonce-account nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#suprana-nonce-account)
...
```bash
suprana withdraw-from-nonce-account nonce-keypair.json ~/.config/suprana/id.json 0.5
```
...
> [Full usage documentation](../usage.md#suprana-withdraw-from-nonce-account)
...
```bash
suprana authorize-nonce-account nonce-keypair.json nonce-authority.json
```
...
> [Full usage documentation](../usage.md#suprana-authorize-nonce-account)
...
```bash
$ suprana-keygen new -o alice.json
$ suprana-keygen new -o nonce.json
$ suprana-keygen new -o bob.json
```
...
```bash
$ suprana airdrop -k alice.json 1
1 SOL
```
...
```bash
$ suprana create-nonce-account -k alice.json nonce.json 0.1
...
```bash
$ suprana transfer -k alice.json --blockhash expiredDTaxfagttWjQweib42b6ZHADSx94Tw8gHx11 bob.json 0.01
[2020-01-02T18:48:28.462911000Z ERROR suprana_cli::cli] Io(Custom { kind: Other, error: "Transaction \"33gQQaoPc9jWePMvDAeyJpcnSPiGUAdtVg8zREWv4GiKjkcGNufgpcbFyRKRrA25NkgjZySEeKue5rawyeH5TzsV\" failed: None" })
...
```bash
$ suprana nonce-account nonce.json
balance: 0.1 SOL
minimum balance required: 0.00136416 SOL
...
```bash
$ suprana transfer -k alice.json --blockhash F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 --nonce nonce.json bob.json 0.01
