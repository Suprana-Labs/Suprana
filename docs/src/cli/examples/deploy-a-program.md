---
title: Deploy a Suprana Program with the CLI
pagination_label: "Suprana CLI: Deploy a Program"
sidebar_label: Deploy a Program
---

Developers can deploy on-chain
[programs](https://suprana.net/docs/terminology#program) (often called smart
contracts elsewhere) with the Suprana tools.

To learn about developing and executing programs on Suprana, start with the
[intro to Suprana programs](https://suprana.net/docs/core/programs) and then dig
into the details of
[developing on-chain programs](https://suprana.net/docs/programs).

To deploy a program, use the Suprana tools to interact with the on-chain loader
to:
...
```bash
suprana program deploy <PROGRAM_FILEPATH>
```
...
```bash
suprana program deploy --program-id <KEYPAIR_FILEPATH> <PROGRAM_FILEPATH>
```
...
```bash
suprana program show <ACCOUNT_ADDRESS>
```
...
```bash
suprana program deploy --max-len 200000 <PROGRAM_FILEPATH>
```
...
```bash
suprana program extend <PROGRAM_ID> <ADDITIONAL_BYTES>
```
...
`suprana-keygen recover` and the following 12-word seed phrase:
==================================================================================
valley flat great hockey share token excess clever benefit traffic avocado athlete
==================================================================================
To resume a deploy, pass the recovered keypair as
the [BUFFER_SIGNER] to `suprana program deploy` or `suprana program write-buffer'.
Or to recover the account's lamports, pass it as the
[BUFFER_ACCOUNT_ADDRESS] argument to `suprana program drain`.
==================================================================================
```
...
```bash
suprana-keygen recover -o <KEYPAIR_PATH>
```
...
```bash
suprana program deploy --buffer <KEYPAIR_PATH> <PROGRAM_FILEPATH>
```
...
```bash
suprana program show --programs
suprana program show --buffers
```

To specify a different authority:

```bash
suprana program show --programs --buffer-authority <AUTHORITY_ADDRESS>
suprana program show --buffers --buffer-authority <AUTHORITY_ADDRESS>
```

To close a single account:

```bash
suprana program close <BADDRESS>
```

To close a single account and specify a different authority than the default:

```bash
suprana program close <ADDRESS> --buffer-authority <KEYPAIR_FILEPATH>
```

To close a single account and specify a different recipient than the default:

```bash
suprana program close <ADDRESS> --recipient <RECIPIENT_ADDRESS>
```

To close all the buffer accounts associated with the current authority:

```bash
suprana program close --buffers
```

To show all buffer accounts regardless of the authority

```bash
suprana program show --buffers --all
```
...
```bash
suprana program deploy --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> <PROGRAM_FILEPATH>
```

Or after deployment and using the default keypair as the current authority:

```bash
suprana program set-upgrade-authority <PROGRAM_ADDRESS> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```

Or after deployment and specifying the current authority:

```bash
suprana program set-upgrade-authority <PROGRAM_ADDRESS> --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```
...
```bash
suprana program deploy <PROGRAM_FILEPATH> --final
```

Or anytime after:

```bash
suprana program set-upgrade-authority <PROGRAM_ADDRESS> --final
```
...
```bash
suprana program dump <ACCOUNT_ADDRESS> <OUTPUT_FILEPATH>
```
...
```bash
$ suprana dump <ACCOUNT_ADDRESS> dump.so
...
```bash
suprana program write-buffer <PROGRAM_FILEPATH>
```
...
```bash
suprana program write-buffer <PROGRAM_FILEPATH> --buffer-authority <BUFFER_AUTHORITY_SIGNER>
```
...
`suprana program set-buffer-authority`:

```bash
suprana program set-buffer-authority <BUFFER_ADDRESS> --new-buffer-authority <NEW_BUFFER_AUTHORITY>
```
...
```bash
suprana program deploy --program-id <PROGRAM_ADDRESS> --buffer <BUFFER_ADDRESS>
```
...
suprana program set-buffer-authority <BUFFER_PUBKEY> --new-buffer-authority <OFFLINE_SIGNER_PUBKEY>
```
...
suprana program upgrade <BUFFER_PUBKEY> <PROGRAM_ID> --sign-only --fee-payer <ONLINE_SIGNER_PUBKEY> --upgrade-authority <OFFLINE_SIGNER> --blockhash <VALUE>

# (5) (use online machine) use this signature to build and broadcast the upgrade transaction on-chain
suprana program upgrade <BUFFER_PUBKEY> <PROGRAM_ID> --fee-payer <ONLINE_SIGNER> --upgrade-authority <OFFLINE_SIGNER_PUBKEY> --blockhash <VALUE> --signer <OFFLINE_SIGNER_PUBKEY>:<OFFLINE_SIGNER_SIGNATURE>
```
Note:
