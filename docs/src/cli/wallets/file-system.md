...
Suprana CLI tools. A file system wallet exists as an unencrypted keypair file
...
Make sure you have
[installed the Suprana Command Line Tools](../install.md)

## Generate a File System Wallet Keypair

Use Suprana's command-line tool `suprana-keygen` to generate keypair files. For
example, run the following from a command-line shell:

```bash
mkdir ~/my-suprana-wallet
suprana-keygen new --outfile ~/my-suprana-wallet/my-keypair.json
```
...
```bash
suprana-keygen pubkey ~/my-suprana-wallet/my-keypair.json
```
...
`~/my-suprana-wallet/my-keypair.json`. The public key of the keypair file is
...
```bash
suprana-keygen verify <PUBKEY> ~/my-suprana-wallet/my-keypair.json
```
