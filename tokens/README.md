...
```bash
suprana-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
suprana-tokens transaction-log --output-path transactions.csv
```
...
```bash
suprana-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
suprana-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
suprana-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
suprana-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
suprana-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `suprana-tokens balances`.
...
```bash
suprana-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
