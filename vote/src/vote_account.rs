use {
    ...
    suprana_vote_program::vote_state::VoteState,
    std::{
        cmp::Ordering,
...
impl TryFrom<AccountSharedData> for VoteAccountInner {
    type Error = Error;
    fn try_from(account: AccountSharedData) -> Result<Self, Self::Error> {
        if !suprana_vote_program::check_id(account.owner()) {
...
mod tests {
    use {
        ...
        bincode::Options,
        rand::Rng,
        suprana_sdk::{pubkey::Pubkey, sysvar::clock::Clock},
        suprana_vote_program::vote_state::{VoteInit, VoteStateVersions},
